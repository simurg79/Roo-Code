...
_A sixth candidate (PAW — Phased Agent Workflow) was evaluated in Phase 10 and does not change this recommendation; see § 7.1._

---

## § 2 — The Question

The user's verbatim ask, restated: _"How do I leave Roo-Code entirely and recreate the same experience using GitHub Copilot — the Chat extension and/or the CLI — preserving my existing [`roo-vault`](../../../../roo-vault) multi-project layout (17 modes, 7 MCP servers, layered rules, symlink-and-commit pattern) on a Windows 11 dev box?"_
...
## § 6 — The Six Migration Paths

[`70-migration-paths.md`](70-migration-paths.md) scores all six against 8 weighted criteria (capability fidelity 30%, effort 20%, operational risk 15%, day-to-day UX 10%, automation 10%, portability 5%, reversibility 5%, cost 5%). Phase 10 added Path E and Path Hybrid+PAW; the ranking still puts plain Hybrid on top.
...
| Hybrid+PAW | Hybrid + PAW skills layer | Hybrid substrate + PAW agents/skills installed on both surfaces; ½-point hit on lock-in (C7) for spec-driven workflow discipline — _Stage-3 escalation_ |     3.85 |    2nd |
| B          | CLI only                  | Terminal-first; `preToolUse` hook closes G-1; loses webview UX                                                                                          |     3.70 |    3rd |
| D          | Vault-as-VSIX             | Custom VS Code extension via `@github/copilot-sdk`; closes G-1/G-2 cleanly at ~1 engineer-month cost                                                    |     3.25 |    4th |
| C          | CLI + Squad               | Path B + Squad's parallel orchestration; alpha-stability tax (CG-12)                                                                                    |     3.20 |    5th |
| A          | Chat only                 | IDE-native; G-1 unmitigated; no headless                                                                                                                |     3.10 |    6th |
| E          | PAW alone                 | PAW (Phased Agent Workflow) on top of one host; forces 17-vault-mode rewrite into 2 PAW agents                                                          |     2.60 |    7th |

**Winner:** **Path Hybrid (3.90)**, beating the runner-up (Path Hybrid+PAW) by 0.05 points and the structural runner-up (Path B) by 0.20 points — a margin driven by Hybrid's parity-or-better score on every criterion (no value below 3) and the structural fact that the CLI is _additive_, not exclusive, with respect to the Chat surface. Path E (2.60) is the worst path investigated and is **not** recommended; PAW's value, if any, only emerges when layered on top of Hybrid (see § 7.1).
...
### § 7.1 — PAW Evaluation _(Phase 10 addendum)_

PAW (Phased Agent Workflow) is, in its own words from [`35-paw-inventory.md` § TL;DR](35-paw-inventory.md), _"a two-agent prompt-and-skill bundle that turns GitHub Copilot Chat / Copilot CLI / Claude Code into a structured, artifact-driven multi-phase development workflow (Spec → Research → Plan → Implement → Review → PR)."_ It ships ~30 `SKILL.md` packages, two `.agent.md` orchestrators (`PAW`, `PAW-Review`), and zero runtime model client of its own — all execution is delegated to the host.

**Why Path E (PAW alone) scored 2.60.** The vault's value is its **17-mode taxonomy**; PAW collapses that into **2 agents + skills + per-work-item `WorkflowContext.md` configuration**. Adopting PAW standalone forces a substantial vault rewrite (C1 = 2), introduces per-work-item Spec/Research/Plan/Plan-Review overhead on every task (C2 = 2), and keeps no headless invocation surface (C5 = 3). It is the worst-scoring path investigated.

**Why Path Hybrid+PAW scored 3.85, not higher.** Layering PAW on top of plain Hybrid keeps every Hybrid win and adds PAW's three structural strengths — serial-by-design workflow state machine, artifact persistence at `.paw/work/<id>/` (the only path with cross-machine, git-portable session state), and multi-model planning + Society-of-Thought review. But it pays a 0.05-point cost on **C7 reversibility** because PAW-managed work items create a vault subtree (`.paw/work/`, `WorkflowContext.md`) that doesn't trivially port back to Roo / Squad / vanilla Copilot, and inherits PAW's 0.x dependency cadence (CLI 0.0.1, extension 0.0.2-dev, plugin 0.3.0) plus dual-tool maintenance overhead. **The honest framing is a rubric limitation, not a PAW deficiency:** the Phase-7 rubric was designed for the vault's 17-mode pattern, and PAW's three structural wins live in a column the rubric doesn't have. A hypothetical C9 criterion at 10% weight for "workflow discipline / artifact code-reviewability" would push Hybrid+PAW (~3.97) above plain Hybrid (~3.71).

**When to escalate to Hybrid+PAW** (Stage-3 escalation analogous to Squad on Path C; full criteria in [`70 § 4.1.1`](70-migration-paths.md#-411--when-to-layer-paw-on-top-added-phase-10b)) — adopt **only if at least two** of these hold: (1) the user's daily workflow is dominated by spec-driven feature PRs and would benefit from Spec→Research→Plan→Implement→Review→PR cadence as a default; (2) per-work-item artifact persistence that survives across machines (and host migrations to Claude Code / future tools) via git is genuinely valued; (3) multi-model planning / Society-of-Thought review is worth the 3-model fan-out cost for high-stakes work; (4) the user is actively dogfooding PAW or has tolerance for a 0.x dependency in the daily driver. If none hold, **stay on plain Hybrid (3.90)**.
...
| ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| Native `AGENTS.md` ingestion (W-5; cross-tool with Claude Code, Cursor, Codex, Gemini CLI)                                   | Per-mode `groups[].fileRegex` enforced _in the IDE_ (G-1; CLI hook only)                                                                          |
...
| Parallel sub-agent dispatch (W-7) — Roo's serial enforcement was a _limitation_                                              | Roo-Code's mature `ModesView.tsx` / `McpServerRestriction.tsx` GUI for visual mode CRUD (CG-1)                                                    |
...
| --------- | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| **Q-047** | Vault's actual `fileRegex` usage breadth — how many modes use `fileRegex` _and_ are dispatched as sub-agents from the orchestrator? | Determines whether G-1 stays 🟠 or re-escalates to 🔴 on the CLI side under CG-11                                                           | **Before Stage-1** (audit command in [`80 § 1`](80-migration-playbook.md#-1--pre-migration-checklist) step 1) |
...
| 10b | **Ongoing — conditional.** _Re-evaluate PAW as a Stage-3 escalation if any of the four trigger conditions in [`70 § 4.1.1`](70-migration-paths.md#-411--when-to-layer-paw-on-top-added-phase-10b) become true_ (spec-driven PR cadence dominates, cross-machine artifact persistence becomes valuable, multi-model SoT review demand emerges, or PAW reaches 1.0 stable). Effort: ~½ day to re-score with the updated rubric. | ½ day (conditional) | [`70 § 4.1.1`](70-migration-paths.md#-411--when-to-layer-paw-on-top-added-phase-10b), [`60 § G`](60-gap-analysis.md#g-paw-surface-summary-added-phase-10b--2026-04-30) |
...
| 6   | **PAW evaluated (Phase 10); recommendation unchanged — plain Hybrid (3.90) remains primary; Hybrid+PAW (3.85) documented as Stage-3 escalation** | Path E (PAW alone) scored 2.60 — worst of any path because it forces a 17-vault-mode rewrite into 2 PAW agents. Hybrid+PAW lost 0.05 on C7 reversibility (PAW lock-in via `.paw/work/` subtree) and inherits 0.x dependency cadence. PAW's three structural wins (serial state machine, artifact persistence, multi-model SoT) live in a column the Phase-7 rubric doesn't have — flagged as a rubric limitation, not a PAW deficiency | Flipping to Hybrid+PAW as default (negative C7 / dual-tool overhead with no rubric-captured gain); adopting Path E standalone (vault rewrite for unproven workflow fit) | 2026-04-30 02:05         |
...
| ------------------------------------------------------------ | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------: |
| [`README.md`](README.md)                                     | ✅ index                        | Phase status badges, file map, usage rules                                                                                                                                       |          ~100 |
| [`00-executive-summary.md`](00-executive-summary.md)         | ✅ Phase 9 + Phase 10 addendum  | **This file — START HERE**                                                                                                                                                       |          ~400 |
...
| [`35-paw-inventory.md`](35-paw-inventory.md)                 | ✅ Phase 10a                    | PAW (Phased Agent Workflow) inventory: 2 agents + ~30 skills + 23-field `WorkflowContext.md`; preliminary Gap Catalog (P-/PW-)                                                   |          ~640 |
...
| [`60-gap-analysis.md`](60-gap-analysis.md)                   | ✅ Phase 6 + Phase 10b          | Unified gap matrix (~70 rows × 12 sections); severity tally; top-10 callout; **PAW column on all 12 sub-tables + § G PAW Surface Summary**                                       |          ~340 |
| [`70-migration-paths.md`](70-migration-paths.md)             | ✅ Phase 7 + Phase 10b          | A/B/C/D + Hybrid scoring against 8 weighted criteria; **Path E (PAW alone) and Path Hybrid+PAW added in §§ 2.F / 2.G; § 4.1.1 trigger conditions**; recommendation = Path Hybrid |          ~685 |
| [`80-migration-playbook.md`](80-migration-playbook.md)       | ✅ Phase 8                      | Concrete file-by-file Hybrid migration; 12 sections + Appendix B; 24-row validation matrix (Hybrid+PAW execution detail deferred to Stage-3 escalation)                          |        ~1,770 |
| [`90-decision-log.md`](90-decision-log.md)                   | ✅ closed (re-closed Phase 10c) | Append-only ADRs (~17 entries); investigation closed by Phase 9 retrospective and re-closed after Phase 10 PAW evaluation                                                        |          ~810 |
| [`99-open-questions.md`](99-open-questions.md)               | ✅ active                       | Q-001..Q-066; resolved items struck through with link to resolving entry; Q-059..Q-066 added during Phase 10                                                                     |          ~145 |

**Total investigation footprint:** ~8,200 lines of memory across 14 files, all cross-linked, all citing primary sources.
